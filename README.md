...
Python library for intagration with WhatsAPP messanger via API of [api.basis-api.com](https://api.basis-api.com) service. To use the library you have to get a registration token and an account id in the [personal area](https://console.api.basis-api.com). There is a free developer account tariff plan.

## API

You can find REST API documentation by [url](https://api.basis-api.com/docs/api/). The library is a wrapper for REST API, so the documentation at the above url applies to the library as well.
...
To send a message or to exacute some other Basic-API method, you have to have the WhatsApp account in the phone application to be authorized. To authorize your account please go to the [personal area](https://console.api.basis-api.com) and scan a QR-code using the WhatsApp application.
